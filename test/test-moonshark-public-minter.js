...
describe("MoonShark NFT Minter", function () {
  ...
  it("Batch Mint - 10", async function () {
    await minterContract.connect(odko).batchMint(10,{ value: ethers.utils.parseEther((0.025*10).toString()) })
  ...
  it("Batch Mint - revert message 'REACHED MINT AMOUNT LIMIT' ", async function () {
    await minterContract.connect(odko).batchMint(9,{ value: ethers.utils.parseEther((0.025*9).toString()) })
    await expect(minterContract.connect(odko).batchMint(2,{ value: ethers.utils.parseEther((0.025*2).toString()) }))
      .to.be.revertedWith("REACHED MINT AMOUNT LIMIT");
  })

  it("Batch Mint - revert message 'MAX_CAP REACHED' ", async function () {
    await minterContract.connect(odko).batchMint(10,{ value: ethers.utils.parseEther((0.025*10).toString()) })
    await minterContract.connect(bilguun).batchMint(10,{ value: ethers.utils.parseEther((0.025*10).toString()) })
  ...
  it("RetrieveFund", async function () {
    ...
    let amountToAdd = ethers.utils.parseEther((0.025*20).toString());
